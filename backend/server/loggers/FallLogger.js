...
const spreadsheetId = '1K-7NaTVRygbOCw0inRv-4137DVvnoqfhgIdN-cVFyXM'
...
function mapName(names, csvName) {
    ...
    names?.forEach((sheetName, index) => {
        ...
        if (cleanedCsvFullName[0] === 'jian') {
            matchingSheetName = 'Jian Chen'
        }
        if (cleanedCsvFullName[0] === '(xiuyi)jasmine') {
            matchingSheetName = 'Xiuy Li'
        }
    })
...
async function main() {
    ...
    // let existingData = (await google.get(groupNum, `I0:I1000`)).data.values
    // let a = existingData.slice(rowNum-1, rowNum+2)
    ...
    for (let i = 0; i < attendanceFiles.length; i++) {
        ...
        //if(csvFileName !== 'd10-05.csv') continue
