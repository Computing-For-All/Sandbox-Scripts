...
let gradesPath = './fall-22-attendance'
let sheetPrefix = 'Group'
let outputPath = './output/attendanceData.csv'
...
const SINGLE_FILE_NAME = SINGLE_FILE_MODE ? '10-21.csv' : ''
...
function appendAttendance(fileName, date, studentsInGroup) {
  ...
  studentsInGroup.forEach(studentName => {
    let timeStamp = { clockIn: '', clockOut: '', duration: 0, status: CLASS_DAYS.includes(dayOfWeek)  ? "Present" : 'Absent' }
